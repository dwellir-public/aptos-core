...
impl SubmissionWorker {
    ...
    pub(crate) async fn run(mut self) -> Vec<LocalAccount> {
        ...
        while !self.stop.load(Ordering::Relaxed) {
            ...
            // we only NEED to check for seq_num for transactions on the LocalAccounts,
            // seqnum for rest will not be updated anyways
            // let all_len = account_to_start_and_end_seq_num.len();
            account_to_start_and_end_seq_num = if self
                .accounts
                .iter()
                .any(|acc| account_to_start_and_end_seq_num.contains_key(&acc.address()))
            {
                ...
                account_to_start_and_end_seq_num
                    .into_iter()
                    .filter(|(acc, _v)| account_addresses.contains(acc))
                    .collect()
            } else {
                let first = account_to_start_and_end_seq_num.into_iter().next().unwrap();
                HashMap::from([first])
            };
    ...
    match client.submit_batch_bcs(txns).await {
        Err(e) => {
            stats
                ...
                .fetch_add(txns.len() as u64, Ordering::Relaxed);
            sample!(
                SampleRate::Duration(Duration::from_secs(20)),
                warn!(
        ...
        Ok(v) => {
            ...
            sample!(SampleRate::Duration(Duration::from_secs(20)), {
